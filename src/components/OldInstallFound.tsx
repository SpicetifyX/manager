...
export default function OldInstallFound({
  fetchInstall,
}: {
  fetchInstall: () => Promise<void>;
}) {
  ...
  useEffect(() => {
    const handleRestoreComplete = (
      _event: any,
      { success, error: restoreError }: { success: boolean; error?: string },
    ) => {
      setRestoring(false);
      if (success) {
        ...
        setError(
          restoreError || "An unknown error occurred during restoration.",
        );
      }
    };
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col items-center justify-center space-y-4 rounded-lg border border-[#2a2a2a] bg-[#121418] p-6">
      ...
      <p className="text-sm text-[#b0b0b0]">
        A previous Spicetify installation was detected. You can restore your
        backup and reset to default configuration.
      </p>
      <button
      ...
      {error && (
        <div className="mt-2 w-full rounded-lg bg-[#3c1212] p-2 text-center text-xs text-[#ff9999]">
