...
import { fetchAppManifest, fetchExtensionManifest, getTaggedRepos } from "../../utils/fetchRemotes";
...
export default function AppsMarketplace() {
  ...
  const appsRef = useRef<app.AppInfo[]>([]);
        ...
        if (result.status === "fulfilled" && result.value.length) {
          apps.push(
            ...result.value.map((ext: any) => ({
              ...ext,
              installed: currentApps.some((a) => a.name === ext.title),
            })),
          );
        }
      }
      setCommunityApps((prev) => (targetPage === 1 ? apps : [...prev, ...apps]));
      setPage(targetPage);
  ...
  useEffect(() => {
    if (communityApps.length > 0) {
      setCommunityApps((prev) =>
        prev.map((ce) => ({
          ...ce,
          installed: apps.some((a) => a.name === ce.title),
        })),
      );
    }
  }, [apps]);
  ...
  const filteredExtensions = useMemo(() => {
    let result = communityApps.map((ext, idx) => ({ ext, origIdx: idx }));
    if (searchQuery.trim()) {
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex h-full fixed w-[calc(100%-4rem)] flex-col overflow-hidden">
                      ...
                      {ext.installed ? (
                        ...
                        <div className="absolute hidden h-full w-full rounded-t-lg bg-gradient-to-b from-black/75 to-black/5 transition-all duration-200 group-hover:block">
                          <div className="flex w-full items-center justify-end gap-1 pt-2 pr-2">
                            ...
                            <button
                              onClick={() => handleInstallApp(ext, origIdx)}
          ...
          {infoIndex !== null &&
            ...
            (() => {
              const ext = communityApps[infoIndex];
