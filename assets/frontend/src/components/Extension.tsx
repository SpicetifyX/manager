...
export default function Extension({ extension }: { extension: app.AddonInfo }) {
  ...
  async function onToggle(addonFileName: string, enabled: boolean) {
    setIsToggling(true);
    await backend.ToggleSpicetifyExtension(addonFileName, enabled);
      ...
      <div className="flex w-full items-center justify-between border-b border-[#2a2a2a] px-4 py-3 transition-colors duration-200 hover:bg-[#1e2228]">
        <div className="flex min-w-0 flex-grow items-center">
          <div className="mr-4 h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg">
            <StaticImage src={extension.preview || extension.imageURL} alt={`${extension.name} preview`} className="h-full w-full object-cover" />
          </div>
          <div className="min-w-0">
            <h3 className="truncate text-lg font-semibold text-white">{extension.name}</h3>
            <p className="truncate text-sm text-[#a0a0a0]">{extension.description}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => onDelete(extension.addonFileName)}
            disabled={isToggling}
          ...
          <label className="flex cursor-pointer items-center gap-3">
            ...
            <div className="relative">
              <input
                ...
                className="sr-only"
                checked={isEnabled}
                onChange={(e) => onToggle(extension.addonFileName, e.target.checked)}
                disabled={isToggling}
              />
