...
export default function AddonCard({
  ...
  return (
    <div
      className={`flex items-center justify-between rounded-lg border p-4 transition-all ${addon.isEnabled ? "border-[#d63c6a] bg-[#1a1418]" : "border-[#2a2a2a] bg-[#121418] hover:border-[#3a3a3a]"
        }`}
    >
      <div className="flex flex-1 items-center gap-4">
        {addon.preview ? (
          <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#0c0e11]">
            <img src={addon.preview} alt={addon.name} className="h-full w-full object-cover" />
          </div>
        ) : (
