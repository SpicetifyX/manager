...
export default function Dashboard({
  ...
  const [reloadModal, setReloadModal] = useState<{ isApplying: boolean } | null>(null);

  const handleReload = async () => {
    setReloadModal({ isApplying: true });
    await window.electron.reloadSpicetify();
    setReloadModal({ isApplying: false });
    fetchVersions();
  };
  ...
  return (
    <div className="flex h-full flex-1 flex-col overflow-x-hidden overflow-y-auto bg-[#171b20] p-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={loading || reloadModal !== null}
                  className="flex items-center gap-2 rounded-lg bg-[#1e2228] px-4 py-2 text-sm font-semibold text-[#a0a0a0] transition-all duration-200 hover:bg-[#2a2e34] hover:text-white disabled:opacity-50"
                >
                  <FaSync className={reloadModal?.isApplying ? "animate-spin" : ""} />
                  Reload
                </button>
                <button
      ...
      {reloadModal && (
        <ApplyModal
          action="Reloading Spicetify"
          items={["spicetify apply"]}
          isApplying={reloadModal.isApplying}
