...
ipcMain.handle(
  ...
  async (_, themeId: string): Promise<boolean> => {
    try {
      console.log(
        `[apply-spicetify-theme] Attempting to apply theme: ${themeId}`,
      );
      ...
      try {
        ...
        const themesManifest: ThemeManifestEntry[] =
          JSON.parse(manifestContent);

        const themeEntry = themesManifest.find((t) => {
          const tid = path.dirname(
            t.usercss || t.schemes || (t.include && t.include[0]) || t.name,
          );
          return tid === themeId;
        });
        ...
        if (themeEntry) {
          ...
          const copyFile = async (relativePath: string | undefined) => {
            ...
            try {
              ...
              console.warn(
                `[apply-spicetify-theme] Could not copy ${relativePath}:`,
                e,
              );
            }
          };
          ...
          await copyFile(themeEntry.schemes);
          ...
          console.log(
            `[apply-spicetify-theme] Theme files for "${themeId}" copied to ${destThemeDir}`,
          );
        }
      } catch (err) {
        console.warn(
          `[apply-spicetify-theme] Could not migrate theme files:`,
          err,
        );
      }
      ...
      try {
        ...
        if (schemeMatch && schemeMatch[1]) {
          ...
          console.log(
            `[apply-spicetify-theme] Found first color scheme: "${firstScheme}"`,
          );
        }
      } catch (e) {
        console.log(e);
        console.log(
          `[apply-spicetify-theme] No color.ini found for ${themeId}, using empty scheme`,
        );
      }
      ...
      console.log(
        `[apply-spicetify-theme] Theme ${themeId} applied successfully.`,
      );

      return true;
    } catch (error) {
      console.error(
        `[apply-spicetify-theme] Failed to apply theme ${themeId}:`,
        error,
      );
      return false;
    }
