...
ipcMain.handle("install-marketplace-app", async (_, user: string, repo: string, appName: string, meta?: MarketplaceAppMeta): Promise<boolean> => {
  try {
    ...
    if (releaseRes.ok) {
      ...
      archiveUrl = zipAsset ? zipAsset.browser_download_url : release.zipball_url;
      console.log(`[install-marketplace-app] Using release archive`);
    } else {
      console.log(`[install-marketplace-app] No release found, scanning branches for built .js files`);

      const branchesRes = await fetch(`https://api.github.com/repos/${user}/${repo}/branches`, { headers: ghHeaders });
      if (!branchesRes.ok) {
        throw new Error(`Failed to fetch branches: HTTP ${branchesRes.status}`);
      }
      const branches = (await branchesRes.json()) as { name: string }[];

      let foundBranch: string | null = null;
      for (const b of branches) {
        const treeRes = await fetch(`https://api.github.com/repos/${user}/${repo}/git/trees/${b.name}?recursive=1`, { headers: ghHeaders });
        if (!treeRes.ok) continue;
        const tree = (await treeRes.json()) as {
          tree: { path: string; type: string }[];
        };
        const jsFiles = tree.tree.filter((entry) => entry.type === "blob" && entry.path.endsWith(".js"));
        if (jsFiles.length > 0) {
          foundBranch = b.name;
          ...
          console.log(`[install-marketplace-app] Found .js files in branch "${b.name}"${jsSubdir ? ` subdir "${jsSubdir}"` : ""}`);
          break;
        }
      }

      if (!foundBranch) {
        throw new Error(`No release and no branch with built .js files found in ${user}/${repo}`);
      }

      archiveUrl = `https://api.github.com/repos/${user}/${repo}/zipball/${foundBranch}`;
    }

    console.log(`[install-marketplace-app] Downloading archive from ${archiveUrl}`);

    const archiveRes = await fetch(archiveUrl, { headers: ghHeaders });
    if (!archiveRes.ok) {
      throw new Error(`Failed to download archive: HTTP ${archiveRes.status}`);
    }
    ...
    console.log(`[install-marketplace-app] Copied ${fileCount} files to ${destDir}`);
